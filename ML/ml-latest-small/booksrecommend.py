...
df = pd.merge(ratings, books, on='ISBN')[['User-ID', 'ISBN', 'Book-Rating', 'Book-Title','Image-URL-M']]
df.head()
# movieId가 중간에 빈 값이 많으므로 순차적인 id를 다시 부여한다.
item_enc = LabelEncoder()
df.head()
df['User-ID'] -= 1
df['ISBN'] = item_enc.fit_transform(df['ISBN'])
df['Book-Rating'] /= 10.0   # 0.5 ~ 5.0 --> 0.1 ~ 1.0으로 표준화.
...
df.head()
df.drop(df.loc[df['Book-Rating']<=0.4].index, inplace=True)

print('user 수 =', len(set(df['User-ID'])))
print('movie 개수 =', len(set(df['ISBN'])))
print('user number = {} ~ {}'.format(df['User-ID'].min(), df['User-ID'].max()))
print('movie number = {} ~ {}'.format(df['ISBN'].min(), df['ISBN'].max()))


df['ISBN'].value_counts()
df.index()
b = dict(zip(df['ISBN'],df['ISBN'].value_counts()))
[if ]

# pivoting
UR = np.array(df.pivot_table(values ='Book-Rating', index='User-ID', columns='ISBN'))
