...
# SVM으로 Train 데이터 세트를 학습한다. gamma는 곡선 
# #C는 아웃라이어(이상치) 허용범위이다. 클스록 많이 후하게 쳐준다.(이상치를 포함)(하드마진이라고도 함)
#즉 c가 클스록 이상치도 포함시켜서 오류를 최대한 허용을 안함(하드마진)
# c 값이 늘리면 편향이 줄어들고 분산이 크다(이상치가 존재하므로)
model = SVC(kernel='rbf', gamma=0.2, C=1.0)
model.fit(x_train, y_train)
